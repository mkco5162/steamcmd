 ...
 java -jar $runportmapper -add -externalPort $server_gameport1 -internalPort $server_gameport1 -ip $myip -protocol udp
 java -jar $runportmapper -add -externalPort $server_gameport2 -internalPort $server_gameport2 -ip $myip -protocol udp
 java -jar $runportmapper -add -externalPort $server_gameport3 -internalPort $server_gameport3 -ip $myip -protocol udp
 java -jar $runportmapper -add -externalPort $server_gameport4 -internalPort $server_gameport4 -ip $myip -protocol udp
 }
...
$script:Serverinstall = $script:install + "\start.bat"
Start-Process $script:Serverinstall
Write-Host ""
Write-Host "최초 서버 실행이 완료되면 20초 후 설치가 계속됩니다" -Foregroundcolor "Cyan"
Write-Host ""
Start-Sleep -s 120
Stop-Process -Name "My Game Host"
Stop-Process -Name "7DTD"
 }
...
(Get-Content $script:Filepath1).replace($script:server_BlockDamageAI_line.Line,$script:server_BlockDamageAI_value) | Set-Content $script:Filepath1
Write-Host ""
...
$script:server_DayLightLength_value = "<property name="DayLightLength"					value=`"" + $script:DayLightLength + "`" />"
...
Write-Host ""
$script:server_DropOnDeath = Read-Host "DropOnDeath (사망시 아이템 드랍여부. nothing: 0 everything: 1 toolbelt_only: 2 backpack_only: 3 delete_all: 4) "
$script:server_DropOnDeath_line = Select-String "DropOnDeath" $script:Filepath1
$script:server_DropOnDeath_value = "<property name="DropOnDeath"					value=`"" + $script:DropOnDeath + "`" />"
...
$script:server_MaxSpawnedZombies_value = "<property name="MaxSpawnedZombies"				value=`"" + $script:MaxSpawnedZombies + "`" />"
(Get-Content $script:Filepath1).replace($script:server_MaxSpawnedZombies_line.Line,$script:server_MaxSpawnedZombies_value) | Set-Content $script:Filepath1
Write-Host ""
...
$script:server_MaxSpawnedAnimals_value = "<property name="MaxSpawnedAnimals"				value=`"" + $script:MaxSpawnedAnimals + "`" />"
...
$script:server_ZombieMove_value = "<property name="ZombieMove"						value=`"" + $script:ZombieMove + "`" />"
(Get-Content $script:Filepath1).replace($script:server_ZombieMove_line.Line,$script:server_ZombieMove_value) | Set-Content $script:Filepath1
Write-Host ""
...
$script:server_ZombieMoveNight_value = "<property name="ZombieMoveNight"				value=`"" + $script:ZombieMoveNight + "`" />"
...
$script:server_ZombieFeralMove_value = "<property name="ZombieFeralMove"				value=`"" + $script:ZombieFeralMove + "`" />"
...
$script:server_ZombieBMMove_value = "<property name="ZombieBMMove"					value=`"" + $script:ZombieBMMove + "`" />"
...
$script:server_BloodMoonFrequency_value = "<property name="BloodMoonFrequency"				value=`"" + $script:BloodMoonFrequency + "`" />"
(Get-Content $script:Filepath1).replace($script:server_BloodMoonFrequency_line.Line,$script:server_BloodMoonFrequency_value) | Set-Content $script:Filepath1
Write-Host ""
...
$script:server_BloodMoonEnemyCount_value = "<property name="BloodMoonEnemyCount"			value=`"" + $script:BloodMoonEnemyCount + "`" />"
...
$script:server_LootAbundance_value = "<property name="LootAbundance"					value=`"" + $script:LootAbundance + "`" />"
...
$script:server_LootRespawnDays_value = "<property name="LootRespawnDays"				value=`"" + $script:LootRespawnDays + "`" />"
...
$script:server_AirDropFrequency_value = "<property name="AirDropFrequency"				value=`"" + $script:AirDropFrequency + "`" />"
...
$script:server_AirDropMarker_value = "<property name="AirDropMarker"					value=`"" + $script:AirDropMarker + "`" />"
...
$script:server_PartySharedKillRange_value = "<property name="PartySharedKillRange"			value=`"" + $script:PartySharedKillRange + "`" />"
...
$script:server_PlayerKillingMode_value = "<property name="PlayerKillingMode"				value=`"" + $script:PlayerKillingMode + "`" />"
